...
def input_prompt_1_func(state_update_prompt, feedback):
    # 动态调整任务优先级
    task_priorities = adjust_task_priorities(state_update_prompt, feedback)

    user_prompt_1 = f'''
    You are a central planner directing lifting agents in a warehouse to lift boxes. 
    Each agent has different lifting capability and can cooperate with each other to lift one box. 
    In summation of lifting capability, the agents can lift all boxes. 
    
    Note that the agents can only lift one box at a time. The current task priorities are:
    {task_priorities}

    The current left boxes and agents are:
    {state_update_prompt}

    Specify your action plan in this format: {{"box[1.7V]":"agent[1.5W]", "box[3.0V]":"agent[1.5W], agent[2.5W]"}}.
    '''
...
def adjust_task_priorities(current_state, feedback):
    """
    动态调整任务优先级。

    参数：
    - current_state (dict | str): 当前任务状态（字典或 JSON 字符串）。
    - feedback (dict): 环境反馈。

    返回：
    - task_priorities (dict): 动态调整后的任务优先级。
    """
    # 检查并解析 current_state
    if isinstance(state_update_prompt, str):
        try:
            state_update_prompt = json.loads(state_update_prompt)  # 转换 JSON 字符串为字典
        except json.JSONDecodeError:
            raise ValueError("current_state must be a dictionary or a valid JSON string.")

    if not isinstance(current_state, dict):
        raise TypeError(f"current_state must be a dictionary, but got {type(current_state).__name__}")
    
    if not isinstance(feedback, dict):
        raise TypeError(f"feedback must be a dictionary, but got {type(feedback).__name__}")

    # 优先级逻辑
    base_priorities = {"explore": 3, "transport": 2, "cover": 1}
    weights = {"completion": 1, "urgency": 2, "time_limit": 3}
    task_priorities = {}

    for task, status in current_state.items():
        priority = base_priorities.get(task, 1)

        # 动态调整逻辑
        if not status.get("completed", False):
            priority += weights["completion"]
        if feedback.get(task, {}).get("urgent", False):
            priority += weights["urgency"]
        if feedback.get(task, {}).get("time_limit", float('inf')) < 5:
            priority += weights["time_limit"]
